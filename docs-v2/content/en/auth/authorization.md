...
Before diving into details about authorization, it's important for you to understand what is the actual lifecycle of the request. So you can know what to expect, and how to debug your app at any point.
...
Restify injects the `RestifyApplicationServiceProvidert`. It is injected in your `config/app.php` and it also has an auto discovered provider called `LaravelRestify\LaravelRestifyServiceProvider`.
...
- Then `RestifyApplicationServiceProvider` is booted, this will define the gate, will load repositories and make the auth routes macro. You have full control over this provider.
...
Let's take a closer look at the package global gate:
...
This is the first gate to access the Restify repositories. In a real-life project, you may allow every authenticated user to have access to repositories, and just after that, by using policies you can restrict specific actions. To do so: 
...
<alert type="info">
For the examples below, we will present PostRepository as an example.
</alert>

### Allow restify

Just after Restify detects the repository class, it will invoke this method to check if the given user can load this repository at all. You can check if the user is an admin for some specific repositories, such as:
...
From here, below, each policy corresponds to an exposed Restify route.

The `show` method, corresponds to the following routes:
...
Determine if a specific user has access to the POST route for creating an entity. 

The `store` method, corresponds to the route:
...
The `storeBulk` method, corresponds to the route:
...
### Allow updateBulk
Determine if the user can update multiple entities at once. When you bulk update, this method will be invoked for each entity you're trying to update, and if at least one will return false, none will be updated, and this is because the bulk update is a DB transaction.

The `updateBulk` method, corresponds to the route:
...
The `delete` method, corresponds to the route:
...
In this case, Restify will guess the policy name by the related entity. By doing so, it will be `attachPost`:
...
When detaching a model from another, we should check if the user is able to do that. Let's take detaching posts from a user as an example:
...
{ "posts": [1, 2, 3] }
```
Restify will guess the policy's name by the related entity. In this case, it will be `detachPost`:
...
A common mistake is that sometimes you may define a policy, but you don't attach it to a model in your `app/Providers/AuthServiceProvider.php`. Make sure you have it defined here.
